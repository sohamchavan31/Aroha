...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    async function loadStoredAuth() {
      try {
        const storedToken = await AsyncStorage.getItem('nira_token');
        const storedUser  = await AsyncStorage.getItem('nira_user');
  ...
  async function login(tokenValue, userData) {
    ...
    await AsyncStorage.setItem('nira_user', JSON.stringify(userData));
    setToken(tokenValue);
    setUser(userData);
  }
