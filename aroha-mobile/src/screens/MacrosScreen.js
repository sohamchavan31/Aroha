...
const SLOTS = [
  ...
  { key: 'DINNER',    label: 'Dinner',    icon: 'moon-outline',       color: '#9B59B6' },
  { key: 'SNACK',     label: 'Snack',     icon: 'cafe-outline',       color: '#2E86AB' },
];
...
export default function MacrosScreen() {
  ...
  function openServingModal(meal, slot = null) {
    setPendingMeal(meal);
    setPieceQty(1);
    setServingGrams(String(meal.typicalServing));
    setCustomQty(false);
    setSelectedSlot(slot || getDefaultSlot());
  }
  ...
  async function confirmAdd() {
    ...
    try {
      await client.post('/logs', { mealId: pendingMeal.id, servingGrams: grams, mealSlot: selectedSlot });
      setPendingMeal(null);
      setQuery('');
      setResults([]);
      loadTodayLog();
  ...
  async function saveRecipe() {
    if (!recipeName.trim()) { Alert.alert('Name required', 'Please enter a recipe name.'); return; }
    if (recipeIngredients.length === 0) { Alert.alert('No ingredients', 'Add at least one ingredient.'); return; }
    const totalG = recipeIngredients.reduce((s, i) => s + (Number(i.grams) || 0), 0);
    if (totalG === 0) { Alert.alert('Invalid', 'All ingredient grams are 0.'); return; }
    ...
    try {
      ...
      setRecipeName(''); setRecipeCategory('other'); setRecipeServings(1);
      setRecipeServingUnit('katori'); setRecipeIngredients([]);
  ...
  function addIngredient(meal) {
    setRecipeIngredients(prev => [...prev, { id: Date.now(), meal, qty: '1', grams: String(meal.typicalServing) }]);
    setRecipeSearch(''); setRecipeResults([]);
  }
  ...
  // Group entries by slot (legacy null → SNACK)
  const slotEntries = SLOTS.reduce((acc, { key }) => {
    acc[key] = log.filter(e => (e.mealSlot || 'SNACK') === key);
    return acc;
  }, {});
    ...
    <SafeAreaView style={styles.safe}>
          ...
          <View style={styles.servingSheet}>
            ...
            <View style={styles.slotPickerRow}>
              {SLOTS.map(({ key, label, color }) => {
                ...
                return (
                  <TouchableOpacity
                    key={key}
                    style={[styles.slotChip, sel && { borderColor: color, backgroundColor: color + '22' }]}
                    onPress={() => setSelectedSlot(key)}>
                    <Text style={[styles.slotChipText, sel && { color }]}>{label}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View style={styles.sheetActions}>
              ...
              <TouchableOpacity
                ...
                onPress={confirmAdd}
                disabled={!preview}>
                <Text style={styles.addBtnText}>Add to {SLOTS.find(s => s.key === selectedSlot)?.label}</Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.builderSheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <CustomField label="Recipe Name *" value={recipeName} onChangeText={setRecipeName} placeholder="e.g. Mom's Dal Tadka" />

              <View style={styles.servingsRow}>
                <Text style={styles.customFieldLabel}>Makes</Text>
                <View style={styles.servingsStepper}>
                  <TouchableOpacity style={styles.servingsBtn} onPress={() => setRecipeServings(s => Math.max(1, s - 1))}>
                    <Ionicons name="remove" size={16} color={Colors.text} />
                  </TouchableOpacity>
                  <Text style={styles.servingsNum}>{recipeServings}</Text>
                  <TouchableOpacity style={styles.servingsBtn} onPress={() => setRecipeServings(s => s + 1)}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
                {SERVING_UNITS.map(unit => (
                  <TouchableOpacity key={unit} style={[styles.chip, recipeServingUnit === unit && styles.chipActive]} onPress={() => setRecipeServingUnit(unit)}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
                {CATEGORIES.map(cat => (
                  <TouchableOpacity key={cat} style={[styles.chip, recipeCategory === cat && styles.chipActive]} onPress={() => setRecipeCategory(cat)}>
              ...
              {recipeResults.length > 0 && (
                <View style={styles.ingResultsList}>
                  {recipeResults.map(meal => (
                    <TouchableOpacity key={meal.id} style={styles.ingResultRow} onPress={() => addIngredient(meal)} activeOpacity={0.7}>
                  ...
                  {recipeIngredients.map(item => (
                    <View key={item.id} style={styles.ingRow}>
                      ...
                      {isUnitFood(item.meal) ? (
                        <>
                          <TextInput style={styles.ingQtyInput} keyboardType="numeric" value={item.qty} onChangeText={v => updateIngredientQty(item.id, v, item.meal.typicalServing)} />
                          <Text style={styles.ingUnitLbl}>{item.meal.servingUnit}</Text>
                        </>
                      ) : (
                        <>
                          <TextInput style={styles.ingGramsInput} keyboardType="numeric" value={item.grams} onChangeText={v => updateIngredientGrams(item.id, v)} />
                          <Text style={styles.ingGramsUnit}>g</Text>
                        </>
                      )}
                      <TouchableOpacity onPress={() => removeIngredient(item.id)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
              ...
              <TouchableOpacity style={[styles.saveBtn, savingCustom && { opacity: 0.6 }]} onPress={saveRecipe} disabled={savingCustom} activeOpacity={0.8}>
                {savingCustom ? <ActivityIndicator color={Colors.background} size="small" /> : <Text style={styles.saveBtnText}>Save Recipe</Text>}
              </TouchableOpacity>
      ...
      <ScrollView
        ...
        <View style={styles.searchRow}>
          ...
          <TouchableOpacity style={styles.addCustomBtn} onPress={() => setShowBuilder(true)} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
        ...
        {results.length > 0 && (
          <View style={styles.resultsCard}>
            {results.map(meal => (
              <TouchableOpacity key={meal.id} style={styles.resultRow} onPress={() => openServingModal(meal)} activeOpacity={0.7}>
        ...
        {loadingLog ? (
          <ActivityIndicator color={Colors.accentGold} style={{ marginTop: 16 }} />
        ) : (
          SLOTS.map(({ key, label, icon, color }) => {
            const entries   = slotEntries[key] || [];
            const slotCal   = Math.round(entries.reduce((s, e) => s + e.calories, 0));
              ...
              <View key={key} style={styles.slotSection}>
                ...
                <TouchableOpacity
                  ...
                  <View style={styles.slotHeaderLeft}>
                    <View style={[styles.slotIconBg, { backgroundColor: color + '22' }]}>
                      <Ionicons name={icon} size={15} color={color} />
                    </View>
                    <Text style={styles.slotLabel}>{label}</Text>
                    {entries.length > 0 && (
                      <View style={[styles.slotCountBadge, { backgroundColor: color + '33' }]}>
                        <Text style={[styles.slotCount, { color }]}>{entries.length}</Text>
                      </View>
                    )}
                  </View>
                  <View style={styles.slotHeaderRight}>
                    <Text style={[styles.slotKcal, { color: entries.length > 0 ? color : Colors.textMuted }]}>
                      {slotCal > 0 ? `${slotCal} kcal` : '—'}
                    </Text>
                    <Ionicons name={isOpen ? 'chevron-up' : 'chevron-down'} size={15} color={Colors.textMuted} style={{ marginLeft: 6 }} />
                  </View>
                </TouchableOpacity>
                ...
                {isOpen && (
                  <View style={styles.slotBody}>
                    {entries.length === 0 ? (
                      <Text style={styles.slotEmpty}>Nothing logged yet</Text>
                    ) : (
                      entries.map(item => (
                        <View key={item.id} style={styles.logRow}>
                          <View style={styles.logInfo}>
                            <Text style={styles.logName}>{item.mealName}</Text>
                            <Text style={styles.logMeta}>{item.servingGrams}g · {Math.round(item.calories)} kcal · P {Math.round(item.protein)}g</Text>
                          </View>
...
const styles = StyleSheet.create({
  ...
  // ── Meal Slot Sections ──
  slotSection:    { backgroundColor: Colors.card, borderRadius: 14, borderWidth: 1, borderColor: Colors.cardBorder, marginBottom: 10, overflow: 'hidden' },
  slotHeader:     { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 13 },
  slotHeaderLeft: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  slotHeaderRight:{ flexDirection: 'row', alignItems: 'center' },
  slotIconBg:     { width: 30, height: 30, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  slotLabel:      { fontSize: 14, fontWeight: '700', color: Colors.text },
  slotCountBadge: { borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 },
  slotCount:      { fontSize: 11, fontWeight: '700' },
  slotKcal:       { fontSize: 13, fontWeight: '600' },
  slotBody:       { paddingHorizontal: 14, paddingBottom: 10 },
  slotEmpty:      { fontSize: 13, color: Colors.textMuted, fontStyle: 'italic', paddingVertical: 10, paddingLeft: 4 },

  logRow:  { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, borderTopWidth: 1, borderTopColor: Colors.cardBorder },
  logInfo: { flex: 1 },
  ...
  sheetUnit:    { fontSize: 12, color: Colors.textMuted, marginBottom: 14 },

  slotPickerRow: { flexDirection: 'row', gap: 6, marginBottom: 18 },
  slotChip:      { flex: 1, paddingVertical: 7, borderRadius: 10, borderWidth: 1, borderColor: Colors.cardBorder, backgroundColor: Colors.background, alignItems: 'center' },
  slotChipText:  { fontSize: 11, fontWeight: '700', color: Colors.textSub },
